...
void chmin(db& x, db y) {x = min(x, y);}
...
int main(){
    ...
    rep(s,w2) rep(i,m) if(s>>(n+i)&1) sp[s] /= 2;
    const double INF = 1e18;
    vector<vector<db>> dp(w2, vector<db>(w,INF));
    dp[0][n+m] = 0;
    vector<vector<double>> dist(w,vector<double>(w));
    ...
    int t = (1<<n)-1 | 1<<(n+m);
    double ans = INF;
    rep(s,w2) if((s&t)==t) chmin(ans,dp[s][m+n]);
