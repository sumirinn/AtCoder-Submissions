...
int main(){
    ...
    if(k%2==0){
        ll ans = 0;
        for(int i=0; i+1<k; i+=2) ans += a[i+1]-a[i];
        cout << ans << endl;
        return 0;
    }
    ...
    siz++;
    vl f(siz), b(siz);
    rep(i,siz-1) f[i+1] = a[i*2+1]-a[i*2];
    rep(i,siz-1) f[i+1] += f[i];
    rep(i,siz-1) b[i] = a[(i+1)*2]-a[i*2+1];
    drep(i,siz-1) b[i] += b[i+1];
    ll ans = INF;
    rep(i,siz) chmin(ans,f[i]+b[i]);
    cout << ans << endl;
}