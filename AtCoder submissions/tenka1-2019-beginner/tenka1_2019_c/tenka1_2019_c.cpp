...
int main(){
    ...
    rep(i,n) cntf[i+1] += cntf[i];
    // rep(i,n+1) cout << i << " " << cntf[i] << endl;
    drep(i,n) cntb[i] += cntb[i+1];
    // drep(i,n+1) cout << i << " " << cntb[i] << endl;


    int ans = inf;
    rep(i,n+1) chmin(ans,cntf[i]+cntb[i]);
    cout << ans << endl;
}