...
int main(){
	...
	vc<pll> dist(n,pll(INF,INF));
	dist[0] = pll(0,0);
	auto dfs = [&](auto dfs, int now)->void{
		auto [x,y] = dist[now];
		for(int nex : to[now]){
			if(dist[nex]!=pll(INF,INF)) continue;
			auto [nx,ny] = l[pii(now,nex)];
			dist[nex] = pll(x+nx,y+ny);
			dfs(dfs,nex);
		}
	};
	dfs(dfs,0);
	...
	rep(i,n){
		if(dist[i]==pll(INF,INF)) cout << "undecidable" << endl;
		else cout << dist[i].fi << " " << dist[i].se << endl;
	}
}