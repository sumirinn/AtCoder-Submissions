...
int main(){
    ...
    vector<vector<vector<int>>> f(n+1,vector<vector<int>>(m,vector<int>(2,0)));
    rep(i,m)rep(j,2) f[0][i][j] = j;
    rep(i,n){
        if(t[i]==1){
            rep(j,m){
                rep(k,2){
                    if(a[i][j]==1 && f[i][j][k]) f[i+1][j][k] = 1;
                    else f[i+1][j][k] = 0;
                }
            }
        }
        if(t[i]==2){
            rep(j,m){
                rep(k,2){
                if(a[i][j]==1 || f[i][j][k]==1) f[i+1][j][k] = 1;
                else f[i+1][j][k] = 0;
                }
            }
        }
        if(t[i]==3){
            rep(j,m){
                rep(k,2){
                bool ok = false;
                if(a[i][j]==1 && f[i][j][k]==0) ok = true;
                if(a[i][j]==0 && f[i][j][k]==1) ok = true;
                if(ok) f[i+1][j][k] = 1;
                else f[i+1][j][k] = 0;
                }
            }
