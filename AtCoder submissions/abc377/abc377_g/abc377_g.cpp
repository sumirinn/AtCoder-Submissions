...
struct Trie {
    ...
    vector<int> nearest = vector<int>(200000+5,inf);
    int root;
    Trie() : root(0) {
    ...
    int insert(const string &word, int word_id) {
        ...
        for (int i = 0; i < (int)word.size(); i++) {
            ...
            node_id = next_id;
            int dist = word.size()-i-1;
            res = min(res,nearest[node_id]+dist);
            nearest[node_id] = min(nearest[node_id],dist);
        }
        ++nodes[node_id].common;
    ...
    int insert(const string &word) {
        int res = insert(word, nodes[0].common);
        return res;
    }
};
