...
int main(){
    ...
    int n;
    cin >> h >> w >> n;
    using bar = tuple<int,int,int>;
    vector<vector<bar>> bars(h);
    rep(i,n){
        int r, c, l;
        cin >> r >> c >> l;
        c--;
    ...
    rep(i,h) sort(nall(bars[i]));
    lazy_segtree<S,op,e,F,mapping,composition,id> t(w);

    vector<int> answers(n);
    rep(i,h)for(auto [c,l,id]  : bars[i]){
        int ans = t.prod(c,c+l);
        answers[id] = h - ans;
        t.apply(c,c+l,ans+1);
    }
