...
int main(){
    ...
    rep(ti,t){
        ...
        int res = 0;
        vvi p(n);
        rep(i,n*2) p[a[i]].pb(i);
        rep(i,n*2){
            rep(v,2){
                int ni = p[a[i]][v];
                if(abs(ni-i)<=1) continue;
                if(0<=ni-1 && i+1<n*2 && abs(ni-1-(i+1))>=2){
                    if(a[ni-1]==a[i+1]) res++;
                }
                if(0<=i-1 && ni+1<n*2 && abs(i-1-(ni+1))>=2){
                    if(a[i-1]==a[ni+1]) res++;
                }
                if(0<=i-1 && 0<=ni-1 && abs(i-1-(ni-1))>=2){
                    if(a[i-1]==a[ni-1]) res++;
                }
                if(i+1<n*2 && ni+1<n*2 && abs(i+1-(ni+1))>=2){
                    if(a[i+1]==a[ni+1]) res++;
                }
            }
        }
