...
int main(){
    ...
    while(cnt<k){
        ...
        q.pop();
        auto [x,y,z] = p;
        if(!st.count(TP(x+1,y,z))&&x+1<n){
            q.emplace(f(x+1,y,z),TP(x+1,y,z));
            st.emplace(x+1,y,z);
        }
        if(!st.count(TP(x,y+1,z))&&y+1<n){
            q.emplace(f(x,y+1,z),TP(x,y+1,z));
            st.emplace(x,y+1,z);
        }
        if(!st.count(TP(x,y,z+1))&&z+1<n){
            q.emplace(f(x,y,z+1),TP(x,y,z+1));
            st.emplace(x,y,z+1);
        }
        cnt++;
        if(cnt==k){
