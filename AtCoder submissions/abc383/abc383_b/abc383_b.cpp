...
int main(){
    ...
    int ans = 0;
    rep(si1,h)rep(sj1,w)rep(si2,h)rep(sj2,w){
        if(s[si1][sj1]=='#'||s[si2][sj2]=='#') continue;
        if(si1==si2 && sj1==sj2) continue;
        set<pii> st;
        for(int i=-d; i<=d; i++)for(int j=-d; j<=d; j++){
            if(abs(i)+abs(j)>d) continue;
            if(out_grid(si1+i,sj1+j,h,w)) continue;
            if(s[si1+i][sj1+j]=='#') continue;
            st.insert(pii(si1+i,sj1+j));
        }
        for(int i=-d; i<=d; i++)for(int j=-d; j<=d; j++){
            if(abs(i)+abs(j)>d) continue;
            if(out_grid(si2+i,sj2+j,h,w)) continue;
            if(s[si2+i][sj2+j]=='#') continue;
            st.insert(pii(si2+i,sj2+j));
        }
