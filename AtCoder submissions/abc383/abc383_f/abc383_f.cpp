...
int main(){
    ...
    vvl dp(n+1,vl(x+1,-INF));
    dp[0][0] = 0;
    rep(i,n){
        for(auto idx : id[i]){
            for(int j=x; j>=p[idx]; j--){
                if(dp[i][j-p[idx]]!=-INF){
                    chmax(dp[i+1][j],dp[i][j-p[idx]]+u[idx]+k);
                }
                if(dp[i+1][j-p[idx]]!=-INF){
                    chmax(dp[i+1][j],dp[i+1][j-p[idx]]+u[idx]);
                }
            }
        }

        rep(j,x+1) chmax(dp[i+1][j],dp[i][j]);
    }

    cout << *max_element(nall(dp[n])) << endl;
}